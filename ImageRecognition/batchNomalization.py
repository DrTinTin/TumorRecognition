...
class NetBatchNorm(nn.Module):
    def __init__(self, channel_num):
        ...
        self.conv1_batchnorm = nn.BatchNorm2d(num_features=channel_num)
        self.conv2 = nn.Conv2d(channel_num, channel_num // 2, kernel_size=3, padding=1)
        self.conv2_batchnorm = nn.BatchNorm2d(num_features=channel_num // 2)
        self.fc1 = nn.Linear(8 * 8 * channel_num // 2, 32)
        self.fc2 = nn.Linear(32, 2)

    def forward(self, img):
        out = self.conv1_batchnorm(self.conv1(img))
        out = fun.max_pool2d(torch.tanh(out), 2)
...
model = NetBatchNorm(channel_num=64).to(device=device)
model.train()
