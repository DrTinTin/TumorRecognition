...
class NetRes(nn.Module):
    def __init__(self, channel_num):
        ...
        self.conv3 = nn.Conv2d(channel_num // 2, channel_num // 2, kernel_size=3, padding=1)
        self.fc1 = nn.Linear(4 * 4 * channel_num // 2, 32)
        self.fc2 = nn.Linear(32, 2)

    def forward(self, img):
        out = fun.max_pool2d(torch.relu(self.conv1(img)), 2)
        out = fun.max_pool2d(torch.relu(self.conv2(out)), 2)
        out1 = out
        out = fun.max_pool2d(torch.relu(self.conv3(out)) + out1, 2)
        out = out.view(-1, 4 * 4 * self.channel_num // 2)
...
model = NetRes(channel_num=64).to(device=device)
model.train()
